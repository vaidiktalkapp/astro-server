...
export class PayoutService {
  ...
  async requestPayout(
    ...
    // ✅ Immediately lock the balance in pendingWithdrawal
    await this.astrologerModel.findByIdAndUpdate(astrologerId, {
      $inc: {
        'earnings.pendingWithdrawal': requestDto.amount,
        'earnings.withdrawableAmount': -requestDto.amount,
      },
      $set: {
  ...
  async getPayoutStats(astrologerId: string): Promise<any> {
    ...
    const [totalPayouts, completedPayouts, pendingAmount] = await Promise.all([
      ...
      this.payoutModel.aggregate([
        { $match: { astrologerId: new Types.ObjectId(astrologerId), status: 'completed' } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ]),
      this.payoutModel.aggregate([
        {
