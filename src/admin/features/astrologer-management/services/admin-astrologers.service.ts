...
export class AdminAstrologersService {
  ...
  async updateAstrologerStatus(
    ...
    // ✅ Sync profile completion if activated
    if (status === 'active') {
      const astroDoc = await this.astrologerModel.findById(astrologerId);
      if (astroDoc) {
        await this.checkAndUpdateProfileCompletion(astroDoc);
        await astroDoc.save();
      }
    }
  ...
  async updatePricing(
    ...
    // ✅ NEW: Sync profile completion steps
    astrologer.profileCompletion.steps.pricing = true;
    astrologer.profileCompletion.steps.availability = true; // Auto-complete as working hours are optional
   ...
   */
  async updateGallery(astrologerId: string, adminId: string, gallery: string[]): Promise<any> {
    const astrologer = await this.astrologerModel.findById(astrologerId);
    if (!astrologer) {
      throw new NotFoundException('Astrologer not found');
    }

    const oldGallery = astrologer.gallery || [];
    astrologer.gallery = gallery;
    
    // Check if gallery step is complete (requires at least 4 photos)
    if (astrologer.profileCompletion && astrologer.profileCompletion.steps) {
      astrologer.profileCompletion.steps.gallery = gallery.length >= 4;
    }
    ...
    await this.activityLogService.log({
      adminId,
      action: 'astrologer.gallery_updated',
      module: 'astrologers',
      targetId: astrologerId,
      targetType: 'Astrologer',
      status: 'success',
      changes: {
        before: { gallery: oldGallery },
        after: { gallery: gallery },
      },
    });
    ...
    return {
      ...
      data: { gallery: astrologer.gallery },
    };
  }

  /**
   * Update astrologer custom platform commission rate
   */
  async updateCommissionRate(
    ...
    if (platformCommissionRate !== null && platformCommissionRate !== undefined) {
    ...
    const oldRate = astrologer.earnings?.platformCommissionRate;
    ...
    if (!astrologer.earnings) {
      ...
      astrologer.earnings.platformCommissionRate = platformCommissionRate;
    }

    // Mark schema path as modified since nested object is mutated
    astrologer.markModified('earnings');
    await astrologer.save();

    // Log activity
    await this.activityLogService.log({
      adminId,
      action: 'astrologer.commission_updated',
      module: 'astrologers',
      targetId: astrologerId,
      targetType: 'Astrologer',
      status: 'success',
      changes: {
        before: { platformCommissionRate: oldRate },
        after: { platformCommissionRate: platformCommissionRate },
      },
    });
    ...
    return {
      ...
      data: { platformCommissionRate: platformCommissionRate },
    };
  }
...
4
  ...
  async deleteAstrologer(astrologerId: string, adminId: string, reason?: string): Promise<any> {
    ...
    deletionDate.setDate(deletionDate.getDate() + 2); // Set to 2 days from now
    ...
    const astrologer = await this.astrologerModel.findByIdAndUpdate(
      astrologerId,
      {
        $set: {
          ...
          permanentDeletionAt: deletionDate, // ✅ FIXED: Added missing deletion date
          updatedAt: new Date()
        }
      },
      { new: true },
  ...
  /**
   * ✅ NEW: Helper to sync profile completion status
   */
  private async checkAndUpdateProfileCompletion(astrologer: AstrologerDocument): Promise<void> {
    ...
    // Update basic info step based on existing data
    steps.basicInfo = !!(
      astrologer.name &&
      astrologer.email &&
    ...
    // Update expertise step based on existing data
    steps.expertise = !!(
      astrologer.specializations?.length > 0 &&
      astrologer.languages?.length > 0
    );

    // Check if all steps are complete (ignore gallery as it's not required yet)
    const allStepsComplete =
      steps.basicInfo === true &&
      steps.expertise === true &&
      steps.pricing === true &&
      steps.availability === true;
