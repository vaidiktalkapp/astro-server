...
export class AdminOrdersService {
  ...
  async getOrderStats(): Promise<any> {
    ...
    const [
      ...
      this.orderModel.countDocuments({ status: 'completed', ...matchFilter }),
      this.orderModel.countDocuments({ status: 'cancelled', ...matchFilter }),
      this.orderModel.countDocuments({ status: 'pending', ...matchFilter }),
      this.orderModel.countDocuments({ status: 'active', ...matchFilter }),
      this.orderModel.countDocuments({ status: 'refund_requested', ...matchFilter }),
  ...
  async getAllCalls(query: any) {
    const { page = 1, limit = 20, status, isAi } = query;
    const skip = (page - 1) * limit;
    const filter: any = {};

    if (status) filter.status = status;

    // ✅ FIX: Exclude AI sessions from main calls traction
    if (isAi !== undefined) {
      filter.isAi = isAi === 'true' || isAi === true;
    } else {
      filter.astrologerModel = { $ne: 'AiAstrologerProfile' };
      filter.isAi = { $ne: true };
    }
  ...
  async getAllChats(query: any) {
    ...
    // ✅ FIX: Exclude AI sessions from main chats traction unless requested
    const isAiParam = query.isAi !== undefined;
    const includeAiParam = query.includeAi !== undefined;

    if (isAiParam || includeAiParam) {
      const isAi = query.isAi === 'true' || query.isAi === true;
      const includeAi = query.includeAi === 'true' || query.includeAi === true;

      if (isAi) {
        filter.orderId = /^AI-/;
        filter.astrologerModel = 'AiAstrologerProfile';
      } else if (includeAi) {
        // Return everything (don't add AI exclusion filters)
      } else {
        filter.orderId = { $not: /^AI-/ };
        filter.astrologerModel = { $ne: 'AiAstrologerProfile' };
      }
    } else {
      filter.orderId = { $not: /^AI-/ };
