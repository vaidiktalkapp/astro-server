...
export class ChatController {
  ...
  async initiateChat(
    ...
    return this.chatSessionService.initiateChat({
      ...
      profileId: initiateDto.profileId
    });
  }
  ...
  async getAstrologerConversationMessages(
    ...
    // Fetch the latest session for this order to get overriding intakeProfile if available
    let intakeProfile: any = null;
    const latestSessionResponse = await this.chatSessionService.getLatestSessionForOrder(order.orderId);
    if (latestSessionResponse?.intakeProfile) {
      intakeProfile = latestSessionResponse.intakeProfile;
    }
      ...
      data: {
        ...
        meta: {
          ...
          astrologer: {
            ...
            profileImage: astrologer?.profilePicture || astrologer?.image || '',
            image: astrologer?.image || astrologer?.profilePicture || '',
            currentRate: astrologer?.pricing?.chat,
          },
          user: {
            _id: user?._id,
            name: intakeProfile?.name || user?.name,
            profilePicture: user?.profileImage,
            kundli: {
              name: intakeProfile?.name || user?.name,
              gender: intakeProfile?.gender || user?.gender,
              dateOfBirth: intakeProfile?.dateOfBirth || user?.dateOfBirth,
              timeOfBirth: intakeProfile?.timeOfBirth || user?.timeOfBirth,
    ...
    return {
      ...
      data: {
        ...
        astrologer: {
          ...
          profilePicture: astrologerData.profilePicture || astrologerData.image || '',
          profileImage: astrologerData.profilePicture || astrologerData.image || '',
          image: astrologerData.image || astrologerData.profilePicture || '',
