...
export class OrdersService {
  ...
  async findOrCreateConversationThread(
    ...
    astrologerModel: string = 'Astrologer'
  ...
  async getUserConversations(
    ...
    const [conversations, total] = await Promise.all([
      this.orderModel
        ...
        .populate('astrologerId', 'name displayName profilePicture image isOnline specializations') // Fetch specific fields
    ...
    const formattedConversations = conversations.map(conv => {
      ...
      return {
        ...
        astrologer: {
          _id: astrologer?._id,
          name: astrologer?.name || astrologer?.displayName || 'Unknown',
          profilePicture: astrologer?.profilePicture || astrologer?.image,
  ...
  async completeSession(
    ...
    const sessionRecord = {
      ...
      startedAt: sessionData.startTime || order.startedAt || new Date(),
      endedAt: sessionData.endTime || new Date(),
    ...
    try {
      ...
      // If it's a human astrologer, fetch commission data
      if (order.astrologerModel !== 'AiAstrologerProfile') {
        astrologer = await this.astrologerModel
          .findById(order.astrologerId)
          .select('earnings.platformCommission')
          .lean();
      }

      const commissionRate = astrologer?.earnings?.platformCommission ?? 100; // default 100% platform for AI
      ...
      if (astrologerEarning > 0 && order.astrologerModel !== 'AiAstrologerProfile') {
      ...
      } else if (order.astrologerModel === 'AiAstrologerProfile') {
        this.logger.log(`📈 AI Revenue Recorded: ₹${userSpend} for session ${sessionData.sessionId}`);
      } else if (!astrologer) {
        this.logger.warn(
    ...
    const updateResult = await this.orderModel.updateOne(
      ...
      },
      {
        $set: {
          ...
          'sessionHistory.$.transcript': recordingData.transcript
        }
      }
    );
  ...
  async getOrderDetails(orderId: string, userId: string): Promise<any> {
    const order = await this.orderModel
      ...
      .populate('astrologerId', 'name displayName profilePicture image experienceYears specializations ratings pricing')
      .lean();
  ...
  async getUserOrders(
    ...
    const [orders, total] = await Promise.all([
      this.orderModel
        .find(query)
        .populate('astrologerId', 'name displayName profilePicture experienceYears specializations ratings')
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
